...
public class MessageDAO extends DBContext {
    ...
    // Lấy ID của owner (chỉ chọn role = 'owner')
    public int getOwnerId() {
        String sql = "SELECT TOP 1 id FROM users WHERE role = 'owner' AND status = 'active'";
    ...
    // Lấy danh sách user mà owner đang chat 
    public List<Message> getChatList(int ownerId, String searchUsername, int page, int recordsPerPage) {
    ...
    try (PreparedStatement ps = connection.prepareStatement(sql.toString())) {
        int paramIndex = 1;
        ps.setInt(paramIndex++, ownerId); // LatestMessages: sender_id
        ps.setInt(paramIndex++, ownerId); // LatestMessages: sender_id
        ps.setInt(paramIndex++, ownerId); // LatestMessages: receiver_id
        ps.setInt(paramIndex++, ownerId); // UnreadCount: receiver_id
        ps.setInt(paramIndex++, ownerId); // UnreadCount: sender_id != ownerId
        ps.setInt(paramIndex++, ownerId); // JOIN: sender_id
        ps.setInt(paramIndex++, ownerId); // WHERE: u.id != ownerId
    ...
    // Đếm tổng số user đang chat với owner
    public int getTotalChatUsers(int ownerId, String searchUsername) {
        StringBuilder sql = new StringBuilder(
    ...
    public List<Message> getMessagesBetweenUsers(int ownerId, int userId) {
        List<Message> list = new ArrayList<>();
    ...
    public boolean markAsRead(int ownerId, int userId) {
