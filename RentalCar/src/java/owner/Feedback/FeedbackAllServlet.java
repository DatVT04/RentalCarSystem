 ...
 * @author nguye
 */
@WebServlet(name = "FeedbackAllServlet", urlPatterns = {"/owner/feedbackall"})
public class FeedbackAllServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        request.getRequestDispatcher("/owner/feedback/feedbackAll.jsp").forward(request, response);
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("updateStatus".equals(action)) {
            try {
                ...
                if (updated) {
                    response.sendRedirect(request.getContextPath() + "/owner/feedbackall?success=update");
                } else {
                    response.sendRedirect(request.getContextPath() + "/owner/feedbackall?error=update");
                }
            } catch (Exception e) {
                e.printStackTrace();
                response.sendRedirect(request.getContextPath() + "/owner/feedbackall?error=update");
            ...
            try {
                ...
                if (deleted) {
                    response.sendRedirect(request.getContextPath() + "/owner/feedbackall?success=delete_feedback");
                } else {
                    response.sendRedirect(request.getContextPath() + "/owner/feedbackall?error=delete_feedback");
                }
            } catch (Exception e) {
                e.printStackTrace();
                response.sendRedirect(request.getContextPath() + "/owner/feedbackall?error=delete_feedback");
