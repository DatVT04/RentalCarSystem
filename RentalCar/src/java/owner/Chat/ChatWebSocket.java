 ...
 * @author nguye
 */
@ServerEndpoint("/chat/{ownerId}/{userId}")
public class ChatWebSocket {
    ...
    public void onOpen(Session session, @PathParam("ownerId") String ownerId, @PathParam("userId") String userId) {
        String sessionKey = ownerId + "-" + userId; // Key dựa trên hướng kết nối
        sessions.put(sessionKey, session);
        System.out.println("WebSocket opened for owner " + ownerId + " with user " + userId + ", session key: " + sessionKey);
    }

    @OnClose
    public void onClose(Session session, @PathParam("ownerId") String ownerId, @PathParam("userId") String userId) {
        String sessionKey = ownerId + "-" + userId;
        sessions.remove(sessionKey);
        System.out.println("WebSocket closed for owner " + ownerId + " with user " + userId);
    }

    @OnMessage
    public void onMessage(String message, Session session, @PathParam("ownerId") String ownerId, @PathParam("userId") String userId) throws IOException {
        try {
            ...
            int receiverId = (senderId == Integer.parseInt(ownerId)) ? Integer.parseInt(userId) : Integer.parseInt(ownerId);
